...
{
    m_iSize = 16;
    for (int i = 0; i < 8193; i*=2) {
        std::string path = "image/nombre/" + std::to_string(i) + ".bmp";

        SDL_Surface* sdlSurface = SDL_LoadBMP(path.c_str());
		if (sdlSurface == NULL)
		{
        ...
        m_mTextures.insert(std::pair<int, SDL_Texture*>(i, sdlTexture));
		if (i == 0) {
			i++;
		}
    ...
    for (int i = 0; i < m_iSize; i++) {
        m_oGrid[i] = new Case(m_mTextures, 16 + (i % 4) * 116, 48 + (i / 4) * 116, 100, 100, m_mTextures[0], oWindow);
    }
...
bool Grid::win() {
    ...
    //SDL_Surface* sdlWinTextSurface = TTF_RenderUTF8_Solid(font, "Victoire !!! Continue", { 255, 0, 0 });
    //if (sdlWinTextSurface == NULL)
    //{
    //    std::cout << "Error TTF_RenderUTF8_Solid :" << SDL_GetError();
    //    oWindow->close();
    //}
    //SDL_Texture* sdlTextureWinText = SDL_CreateTextureFromSurface(oWindow->m_sdlRenderer, sdlWinTextSurface);
    //if (sdlTextureWinText == NULL)
    //{
    //    std::cout << "Error SDL_CreateTextureFromSurface :" << SDL_GetError();
    //    oWindow->close();
    //}
    //SDL_FreeSurface(sdlWinTextSurface);
    //GameObject* oLoose = new GameObject(200, 200, 200, 200, sdlTextureWinText, oWindow);
    if (m_iNbMax != 2048) {
        return false;
    }
    return true;
